...
namespace VALORANT
{
	...
	/*namespace Variables {
		uint64 process_event;
		uint64 static_find_object;
		uint64 static_load_object;
		uint64 skin_decrypt;
		uint64 set_ares_outline;
		uint64 bone_matrix;
		uint64 get_spread_values;
		uint64 get_spread_angles;
		uint64 get_firing_location_direction;
		uint64 tovector_and_normalize;
		uint64 toangle_and_normalize;
		uint64 skin_pointer;
		uint64 skin_pointer_2;
		uint64 skin_pointer_3;
	}*/
}
...
struct FName_ 
{
	FName_() : comparison_index(std::int32_t()), number(std::int32_t()) {};
	FName_(std::int32_t index) : comparison_index(index), number(std::int32_t()) {};

	bool operator==(const FName_& other) const noexcept {
...
FNameEntryId FindFNameEntryId(const std::string& target)

{
	for (uint32 block = 0; block < NamePoolData->Entries.NumBlocks(); ++block)
	{
		for (uint32 offset = 0; offset < FNameBlockOffsets; ++offset)
		{
			FNameEntryHandle handle(block, offset);

			FNameEntry& entry = NamePoolData->Entries.Resolve(handle);

			if (entry.GetEntryHeader().Len == 0) continue;

			std::string name = entry.String();

			if (name == target)
			{
				return handle; // implicitly convertible to FNameEntryId
			}
		}
	}
	return FNameEntryId();  // Return a null/invalid ID
